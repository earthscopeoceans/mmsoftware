...
! compile: g7 pltamps delaz radamp ttak135 bin

! Construct a GMT file for gmtPrad to map P wave amplitudes on a globe
! with blue for positive, and red for negative polarity
! Idem for pP (plot with gmtsmallPrad).

! Input is:
! [1] Earthquake lat,lon,depth (deg, km)
! [2] min max latitude
! [3] min max longitude
! [4] Moment tensor elements Mrr,Mtt,Mpp,Mrt,Mrp,Mtp (any scale is OK)
...
real*4 :: xmom(6),p,azi,vsrc,up,radp,radsv,radsh,dist  ! radiation
...
print *,'Give Mrr,Mtt,Mpp,Mrt,Mrp,Mtp (any scale is OK):'
read(5,*,iostat=ios) xmom
if(ios.ne.0) then
  print *,'ERROR in plotamps first Moment tensor input'
  stop
endif  
...
do j=1,nlon
  ...
  do i=1,nlat
    ...
    if(p<0.) cycle
    call radamp(depth,xmom,p,azie,vsrc,up,radp,radsv,radsh)
    prad(i,j)=radp
